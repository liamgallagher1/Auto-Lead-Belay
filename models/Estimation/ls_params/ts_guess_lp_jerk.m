function [theta_hat, omega_hat, alpha_hat, sse_pos, se_omega, se_alpha, se_jerk] =...
    ts_guess_lp_jerk(counts, times, sigma, num_counts, order, curr_time, ...
    omega_prior, omega_weight, alpha_prior, alpha_weight, jerk_weight)
...
% se_jerk : weight of squared jerk over time
...
M_norm = min_jerk_matrix(order, 0, curr_time_n); 
% Real jerk in normalized terms requires dividing by (t_curr - t0)^3. 
% Real squared jerk in normalized terms requires dividing by (t_curr - t0)^3; 
int_horizon_s = curr_time - sub_t(end); 
M_real = M_norm / int_horizon_s^6; 
M_cost = M_norm * jerk_weight / int_horizon_s^6; 
M_cost = M_cost(1:order-2, :); % Mostly zeros

A(end-(order-2)+1:end, :) = M_cost;
% B remains zeros here, no expected jerk
...
A(num_counts+1:num_counts+2, :) = A(num_counts+1:num_counts+2, :) ./ delta_t;
A(num_counts+2, :) = A(num_counts+2, :) ./ delta_t;
...
% unscaled sum of square of jerk
se_jerk = P' * M_real * P;
